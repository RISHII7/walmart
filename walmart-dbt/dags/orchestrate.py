...
def orchestrate():
    ...
    def ingest_cdc():
        ws = WorkspaceClient(
        host="https://dbc-767676bd-cdaa.cloud.databricks.com",
        token="dapi3770084c76542513b557e8e37c08444d"
        )

        job_trigger = ws.jobs.run_now(job_id="481779964039649")
    ...
    @task.bash
    def clean_target():
        return "rm -rf /opt/airflow/walmart/target && rm -rf /opt/airflow/walmart/logs"

    @task.bash
    def source_freshness():
        # Manually set the working directory using the 'cd' command before running the dbt command
        return "cd /opt/airflow/walmart && dbt source freshness"
